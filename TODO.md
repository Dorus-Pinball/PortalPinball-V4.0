...
- [x] Add a GitHub Actions workflow running `python -m unittest discover tests` on every
      push/PR — done 2026-09-13 as `.github/workflows/ci.yml`, folded into the KM-overhaul pass
      once `generate_docs.py` existed to make a doc-freshness check possible too (same workflow
      also runs `check_registry.py` and diffs the generated docs against committed output).
