...
bring-up status — board map, known-component wiring tables, LEDs, known gaps, and harness
diagrams, in one place. **Generated** by `tools/hw_console/generate_docs.py` from
...
Components whose notes flag an explicit `GAP:` in `components.yaml` — see [TODO](/TODO) for full
detail and status:
